...
- 🖥️ **桌面管理界面**：`clipd_ui.py` 提供 GTK 界面管理服务、查看实时日志
...
Ubuntu 端安装后在应用菜单中的名字是 `Clipd Desktop` / `Clipd 桌面端`，但命令名、systemd 服务名和协议名仍然保持 `clipd`。后台守护由 `ubuntu_clipd.py` 提供，GUI 管理器是 `clipd_ui.py`。
...
WiFi 断开/重连后会自动以 0/2/5/10 秒退避重试验证连接，无需手动点「开始同步」。
