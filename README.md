...
ClipSyncService                   ubuntu_clipd.py
 ...
 │   └─ 检测复制 → 触发读取        ├─ wl-paste --watch
 ├─ 临时可获焦 Overlay             │   └─ 监听剪贴板 → POST Android
 │   └─ getPrimaryClip()          ├─ avahi-publish-service
 ├─ ClipboardIMEService           │   └─ mDNS 广播 _clipd._tcp
 │   └─ 备选：IME 豁免读取         └─ UDP Broadcast :8890
 ├─ HTTP Server :8889
 │   └─ 接收 Ubuntu 文字/图片
 └─ NsdManager
     └─ 发现 _clipd._tcp
